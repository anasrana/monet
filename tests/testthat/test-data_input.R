...
test_that("reading ATACseq", {
    ...
    expect_that(ncol(atac_dt), equals(6))
...
test_that("testing with path", {
    monet_data <- monetData_init(gene_path, atac_path, "gene", "Gene")

    expect_is(monet_data, "monetData")
...
test_that("testing monedtData filter", {
    ...
    expect_is(monet_filt, "monetDataFilt")
    expect_equal(length(monet_filt@gene_overlap_list), 8)
    ...
    expect_is(monet_filt, "monetDataFilt")
    expect_equal(length(monet_filt@gene_overlap_list), 13)
    expect_equal(monet_data@no_tpts, 4)
    expect_equal(monet_filt@data_test, TRUE)
    ...
    expect_is(monet_filt, "monetDataFilt")
    expect_equal(length(monet_filt@gene_overlap_list), 5)
    expect_equal(monet_data@no_tpts, 4)
    expect_equal(monet_filt@data_test, TRUE)
